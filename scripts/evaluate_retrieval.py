...
def _matches_expectation(
    ...
    if (version and version not in [v.lower() for v in expected_versions]) or not any(
        company.find(c.lower()) != -1 for c in expected_companies
    ):
        print(
            f"Chunk version '{version}' or company '{company}' does not match expected versions {expected_versions} or companies {expected_companies}"
        )
        return False
    ...
    if expected_page_numbers:
        ...
        if page_number not in expected_page_numbers:
            print(
                f"Chunk page number '{page_number}' not in expected page numbers {expected_page_numbers}"
            )
            return False
    ...
    if expected_substrings:
        ...
        if not any(term.lower() in haystack for term in expected_substrings):
            print(
                f"Chunk does not contain any expected substrings {expected_substrings}"
            )
            return False
...
def main() -> None:
    ...
    for i, item in enumerate(GOLDEN_DATASET, start=1):
        ...
        try:
            chunks, _filters = retrieve_context(
                query, CLIENT_ID, rerank_with_model=False
            )
        ...
        expectation_bits = [
            f"version in {expected_versions}",
            f"company in {expected_companies}",
        ]
        if expected_page_numbers:
        ...
        print(
            f"[{i}/{total}] {status} | query='{query}' | expected={' | '.join(expectation_bits)}\n\n"
        )
